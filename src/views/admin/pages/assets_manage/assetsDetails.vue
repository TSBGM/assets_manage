...
export default {
    ...
    methods: {
        ...
        searchDetails(val){
            ...
            this.$store.dispatch('saveComelninsert',this.assetsDetailsModel)
            .then(res => {
                if(res.code == 100){
                    this.rep = res.data  
        ...
        //詳情   
        seeStampApplyInfo(applyNum,stampUseId){
            localStorage.setItem('applyNum', applyNum);
            localStorage.setItem('stampUseId', stampUseId);
            this.$store.dispatch('particulars', {applyNum:applyNum,relDocId:stampUseId})
            .then(res => {
                // if(res.data.code == 0){
                    // window.open("http://localhost:8080/#/archiveApplicationDetails")
                    window.open(`${BASE_URL}/#/archiveApplicationDetails`)
            })
